...
public class HttpUtils {
    ...
    public void post(String url, Map<String, String> map, StringBuffer result) {
        ...
        try {
            uefEntity = new UrlEncodedFormEntity(formparams, Const.FUYOU_CHARSET);
            httppost.setEntity(uefEntity);

            logger.info("提交请求 " + httppost.getURI());
            CloseableHttpResponse response = httpclient.execute(httppost);
            try {
                HttpEntity entity = response.getEntity();

                if (entity != null && result != null) {
                    result.append(EntityUtils.toString(entity, Const.FUYOU_CHARSET));
                }
